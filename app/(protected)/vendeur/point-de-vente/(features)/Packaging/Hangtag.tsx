...
export default function Hangtag({ hangtag, getDevis, getPrix, activeSection }: HangtagProps) {
    ...
    const [devisHangtag, setDevisHangtag] = useState<devisData>({
        client_id: 0,
        type: hangtag.type,
        couleur_id: 0,
        couleur: '',
    ...
    useEffect( () => {
        getDevis(devisHangtag);
        getPrix(prix.prixTotal, prix.prixUnitaire); 
    }, [devisHangtag, prix.prixTotal, prix.prixUnitaire]);
    ...
    useEffect(() => {
        ...
        /*const dimensionSelectionnee = hangtag.dimensions.find(
            d => d.id === devisHangtag.dimension_id
        );
        ...
        // 2. Prix du matériau
        if (devisHangtag.categorie === 'autres') {
            ...
            const categorieSelectionnee = categories.find(
                c => c.id === devisHangtag.categorie_id
            );
            if (categorieSelectionnee) {
                // Appliquer un coefficient selon la catégorie
                if (devisHangtag.categorie === 'PCB Pélliculé') {
        ...
        devisHangtag.dimension_id,
        devisHangtag.dimension,
        devisHangtag.categorie_id,
        devisHangtag.categorie,
        devisHangtag.materiau_id,
        devisHangtag.couleur_id,
        devisHangtag.recto_verso_id,
        devisHangtag.imprimante_id,
        devisHangtag.decoupe,
        devisHangtag.emplacement,
        devisHangtag.particularite,
        devisHangtag.quantite,
        autreMateriau.prix,
        autreDecoupe.prix,
                ...
                <div className="w-full lg:w-full space-y-1">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {hangtag.dimensions.map(dimension => (
                                    <button
                                        ...
                                        className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 ${devisHangtag.dimension_id === dimension.id ? 'bg-red-600 text-white border-red-600 shadow-md' : 'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}`}
                                    >
                    ...
                    <div className='flex mb-4'>
                        <div ref={materiauRef} className="w-full lg:w-1/2 scroll-mt-20">
                                    ...
                                    {categories.map(categorie => (
                                        <Button key={categorie.id}
                                            ...
                                            className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 
                                                ${devisHangtag.categorie_id === categorie.id ? 'bg-red-600 text-white border-red-600 shadow-md hover:bg-red-600 hover:text-white' :
                            ...
                            {devisHangtag.categorie &&
                                <div className={`${devisHangtag.categorie === 'autres' ? 'hidden' : ''} `}>
                                    <div className='flex ml-5 mb-4 text-gray-800 font-semibold'>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                        {hangtag.matieres.map(materiau => (
                                            <button
                                                key={materiau.id}
                                                onClick={() => {
                                                    handleSelect(materiau.id, 'materiau_id', 'materiau', `${devisHangtag.categorie}-${materiau.taille}`.trim());
                                                }}
                                                className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 
                                                    ${devisHangtag.materiau_id === materiau.id ?
                                                        'bg-red-600 text-white border-red-600 shadow-md' :
                                                        'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}
                                                    ${devisHangtag.categorie === 'PCB Pélliculé' && materiau.taille === '700G' ? "hidden" : ''}
                                                    `}
                                            >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {hangtag.faces.map(face => (
                                    <button
                                        ...
                                        className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 ${devisHangtag.recto_verso_id === face.id ? 'bg-red-600 text-white border-red-600 shadow-md' : 'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}`}
                                    >
                                        <span>{face.face}</span>
                                    </button>
                                ))}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {hangtag.imprimantes.map(imprimante => (
                                    <button
                                        ...
                                        className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 ${devisHangtag.imprimante_id === imprimante.id ? 'bg-red-600 text-white border-red-600 shadow-md' : 'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}`}
                                    >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {hangtag.decoupes.map((decoupe) => (
                                    <button
                                        ...
                                        className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 ${devisHangtag.decoupe === decoupe.decoupe ? 'bg-red-600 text-white border-red-600 shadow-md' : 'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}`}
                                    >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {hangtag.emplacements.map(emplacement => (
                                    <button
                                        ...
                                        className={`p-3 border rounded-lg text-center text-sm transition-all duration-200 ${devisHangtag.emplacement === emplacement.emplacement ? 'bg-red-600 text-white border-red-600 shadow-md' : 'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}`}
                                    >
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {hangtag.particularites.map((part) => (
                                    <button
                                        ...
                                        className={`truncate p-3 border rounded-lg text-center text-sm transition-all duration-200 ${devisHangtag.particularite === part.particularite ? 'bg-red-600 text-white border-red-600 shadow-md' : 'bg-white dark:bg-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-600 hover:border-red-500 dark:hover:border-red-500'}`}
                                    >
                    ...
                    <div className='flex mb-4'>
                        <div ref={quantiteRef} className="w-full lg:w-1/2 scroll-mt-20">
                            ...
                            <Input
                                type="number"
                                value={devisHangtag.quantite?.toString()}
