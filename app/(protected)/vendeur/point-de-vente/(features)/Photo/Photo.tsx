...
export default function Photo({ userRole, param, handleAddCart }: CatalogueProps) {
    // ========== HOOKS PERSONNALISÉS ==========
    const { cadre_photo, photobook, tirage_photo, photosLoading } = usePhotos();
    ...
    const itemData = {
        types: [
            { id: 1, type: 'tirage_photo', nom: 'Tirage Photo' },
            { id: 2, type: 'cadre_photo', nom: 'Cadre Photo' },
            { id: 3, type: 'photobook', nom: 'Photobook' },
        ]
    };
    ...
    const handleAddToCart = () => {
        ...
        const currentItem: CartItemsType = {
            id: Date.now(),
            designation: `Photo - ${textileType?.nom || devisEncours?.type}`,
    ...
    const Products: Record<string, typeof tirage_photo> = {
        tirage_photo,
        cadre_photo,
        photobook,
    };
