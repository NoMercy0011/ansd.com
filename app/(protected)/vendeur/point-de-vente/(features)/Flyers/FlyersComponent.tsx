...
export default function FlyersComponent({ item, getDevis, getPrix, activeSection }: ItemProps) {
    ...
    // --- Extraction dynamique des catégories disponibles ---
    const categoriesDisponibles = useMemo(() => {
        const typesUniques = new Set<string>();
        item.matieres!.forEach(matiere => {
            if (matiere.type) {
                typesUniques.add(matiere.type);
            }
        });

        const categories = Array.from(typesUniques).map((type, index) => ({
            id: index + 1,
            categorie: type
        }));

        // Ajouter l'option "autres" à la fin
        categories.push({ id: categories.length + 1, categorie: "autres" });
        ...
        <div>
            <div className="flex flex-col lg:flex-row gap-3">
                <div className="w-full lg:w-full space-y-1">
                    ...
                    {/* Section Materiaux */}
                    <div className='flex mb-4'>
                        <div ref={materiauRef} className={`w-full transition-all duration-200 scroll-mt-20 ${devisEncours.categorie === "autres" ? 'lg:w-1/2' : 'lg:w-full'}`}>
                            <h4 className="font-semibold text-slate-700 dark:text-slate-200 mb-4 flex items-center">
                                <Layers className="mr-2" />
                                Matériaux
                            </h4>
                        ...
                        <div ref={imprimanteRef} className="w-full lg:w-1/2 scroll-mt-20">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                {item.imprimantes!.map(imprimante => (
                                    ...
                                    >
                                        <div className="font-semibold capitalize">{imprimante.imprimante}</div>
                                    </button>
                                ))}
                            </div>
                        </div>
