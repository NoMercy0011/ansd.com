...
//         setCartItems(prev => prev.map(item => { if (item.id === itemId) { const val = (field === 'quantite' || field === 'prixUnitaire' || field === 'remise') ? parseFloat(value) || 0 : value; return { ...item, [field]: val }; } return item; }));
//     };
...
//                 <div className="w-full lg:w-2/3 space-y-6"><StockItemBrowser stockItems={stockItems} onAddToCart={handleAddToCart} /><OtherServicesBrowser services={otherServices} onAddToCart={handleAddToCart} /><ManualItemEntry onAddToCart={handleAddToCart} title="Ajouter un service de Livraison" icon={Truck} type="Livraison" addNotification={addNotification} /><ManualItemEntry onAddToCart={handleAddToCart} title="Ajouter un article manuel personnalisé" icon={PencilRuler} type="Manuel" addNotification={addNotification} /><ItemConfigurator userRole={userRole} onAddToCart={handleAddToCart} /></div>
...
//             <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden"><table className="w-full">
//                 <thead className="bg-slate-50 dark:bg-slate-700/50"><tr><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">N° Doc</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Type</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Date</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Client</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Total TTC</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Reste</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Statut</th><th className="p-4 text-left text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Action</th></tr></thead>
...
//                     return (
//                     <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"><td className="p-4 text-sm text-slate-500 dark:text-slate-400 font-mono flex items-center gap-2">#{inv.docNumber} {isOverdue && <Clock size={14} className="text-orange-500" title="Paiement en retard"/>}</td><td className="p-4 text-sm font-medium capitalize text-slate-700 dark:text-slate-300">{inv.type}</td><td className="p-4 text-sm text-slate-500 dark:text-slate-400">{new Date(inv.date).toLocaleDateString('fr-FR')}</td><td className="p-4 text-sm font-medium text-slate-900 dark:text-slate-200">{inv.client.name}</td><td className="p-4 text-sm font-semibold text-slate-800 dark:text-slate-200">{inv.totalPrice.toLocaleString('fr-FR')} Ar</td><td className="p-4 text-sm font-bold text-red-600">{inv.resteAPayer.toLocaleString('fr-FR')} Ar</td><td className="p-4"><span className={`px-2 py-1 text-xs font-medium rounded-full ${statusInfo.style}`}>{statusInfo.text}</span></td><td className="p-4 flex items-center gap-1 relative">{inv.resteAPayer > 0 && ( <Button variant="success" onClick={() => onFinalizeInvoice(inv)} className="py-1 px-2 text-xs">Finaliser</Button> )}<Button variant="ghost" icon={Copy} onClick={() => onDuplicateDocument(inv)} className="p-2 h-auto" title="Dupliquer" /><Button variant="ghost" icon={Eye} onClick={() => onPrepareAndPrint(inv)} className="p-2 h-auto" title="Visualiser" /><Button variant="ghost" icon={MoreVertical} onClick={() => setMenuOpen(menuOpen === inv.id ? null : inv.id)} className="p-2 h-auto" />
//                     {menuOpen === inv.id && (
